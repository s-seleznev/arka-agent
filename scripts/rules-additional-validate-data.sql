...
DO $$ DECLARE t record;p record;ctx jsonb;BEGIN
 ...
 FOR t IN SELECT * FROM test_facts LOOP
  ...
  IF field_value_at(t.animal_id,'LAST_MILK_TEST_KG',p.as_of,p.knowledge_at) IS DISTINCT FROM '17.25'::jsonb THEN RAISE EXCEPTION 'milk test selection'; END IF;
  IF (SELECT rule_values->'LAST_MILK_TEST_KG' FROM animal_state_query WHERE animal_id=t.animal_id) IS DISTINCT FROM '17.25'::jsonb THEN RAISE EXCEPTION 'milk projection'; END IF;
