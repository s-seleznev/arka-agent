...
Пользователь ведёт диалог слева и одновременно работает с таблицей животных справа. Аркаша знает текущее сохранённое представление, фактически видимые строки, выделение и раскрытые группы. По команде пользователя агент читает базу фермы, меняет представление таблицы и объясняет результат. Ручные изменения пользователя сразу становятся новым контекстом агента.
...
Номер животного является первой закреплённой колонкой и основной ссылкой строки. Hover ячейки показывает кнопку карточки справа; у кнопки есть собственное hover/focus-состояние. Hover текста подчёркивает только номер. И текст, и кнопка независимо открывают один стандартный `Sheet` справа; кличка не дублирует это действие.
