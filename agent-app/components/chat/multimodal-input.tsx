...
function PureMultimodalInput({
    ...
    (cmd: SlashCommand) => {
      ...
      switch (cmd.action) {
        ...
        case "clear":
          ...
          break;
        case "rename":
          toast("Переименовать чат можно через меню в истории.");
          break;
        case "model": {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const { url, name, contentType, id } = data;

          mutate(
            "/api/files",
            (current: { files?: Record<string, unknown>[] } | undefined) => ({
              files: [
                {
                  contentType,
                  id,
          ...
          window.dispatchEvent(new Event("arkasha-files-changed"));
          return {
            contentType,
            id,
            name,
        ...
        toast.error("Не удалось загрузить файл. Повторите попытку.");
      }
    },
    [mutate]
  );
  ...
  const handlePaste = useCallback(
    async (event: ClipboardEvent) => {
      ...
      setUploadQueue((prev) => [...prev, "Вставленное изображение"]);
      ...
      try {
        ...
        toast.error("Не удалось загрузить вставленное изображение");
      } finally {
        setUploadQueue([]);
      }
    },
  ...
  const handlePromptSubmit = useCallback(() => {
    ...
    if (status === "ready" || status === "error") {
      ...
      toast.error("Дождитесь завершения ответа.");
    }
  ...
  return (
    <div className={cn("relative flex w-full flex-col gap-4", className)}>
      ...
      <input
        accept=".csv,.docx,.json,.md,.pdf,.txt,.xlsx,image/jpeg,image/png,image/webp"
      ...
      <PromptInput
        className="chat-composer [&>div]:rounded-2xl [&>div]:border [&>div]:border-border/30 [&>div]:bg-card/70 [&>div]:shadow-[var(--shadow-composer)] [&>div]:transition-shadow [&>div]:duration-300 [&>div]:focus-within:shadow-[var(--shadow-composer-focus)]"
  ...
  return (
    <ModelSelector onOpenChange={setOpen} open={open}>
      ...
      <ModelSelectorContent commandDefaultValue={selectedModel.id}>
        <ModelSelectorInput placeholder="Найти модель..." />
        <ModelSelectorList>
          {(() => {
