...
export function Console({ consoleOutputs, setConsoleOutputs }: ConsoleProps) {
  ...
  return consoleOutputs.length > 0 ? (
    <>
      ...
      <div
        className={cn(
          "fixed bottom-0 z-40 flex w-full flex-col overflow-x-hidden overflow-y-auto border-t border-border bg-background",
          { "select-none": isResizing }
        )}
        ref={consoleContainerRef}
        style={{ height }}
      >
        <div className="sticky top-0 z-50 flex h-10 w-full items-center justify-between border-b border-border bg-background px-3">
        ...
        <div className="bg-background">
          {[...consoleOutputs].reverse().map((consoleOutput, index) => (
            <div
              className="flex border-b border-border px-4 py-2.5 font-mono text-[12px] leading-relaxed"
