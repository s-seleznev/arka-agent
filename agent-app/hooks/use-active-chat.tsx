...
export function ActiveChatProvider({ children }: { children: ReactNode }) {
  ...
  const { preview: workspacePreview } = useWorkspacePreview();
  const workspacePreviewRef = useRef(workspacePreview);
  useEffect(() => {
  ...
  const [chatTitle, setChatTitle] = useState("Новая задача");
  ...
  useEffect(() => {
    setChatTitle(isNewChat ? "Новая задача" : (chatData?.title ?? "Чат"));
  }, [chatData?.title, isNewChat]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("arkasha-chat-title-changed", handleTitleChange);
    return () =>
      window.removeEventListener(
        "arkasha-chat-title-changed",
        handleTitleChange
      );
  }, [chatId]);
      ...
      prepareSendMessagesRequest(request) {
        ...
        return {
          body: {
            ...
            ...(workspacePreviewRef.current?.view
              ? {
                  viewContext: {
                    expandedGroupPaths:
                      workspacePreviewRef.current.expandedGroupPaths,
                    revision: workspacePreviewRef.current.view.revision,
                    selectedIds: workspacePreviewRef.current.selectedIds,
                    viewId: workspacePreviewRef.current.view.id,
                    viewportRowIds: workspacePreviewRef.current.viewportRowIds,
                  },
                }
              : {}),
            ...request.body,
